...
if(isset($_GET['material_type_id'])){
    ...
    $stmt = $conn->query("SELECT * FROM `material_types` WHERE `material_type_id`= '".$material_type_id."' ");
...
<div id="container">
    ...
    <div id="content">
        <div class="row">
            <form class="col s12" method="post" action="material_update.php">
                <div class="input-field col s4">
                    <input required type="hidden" name="material_type_id" value="<?php echo $material_type_id ?>">
                    <input id="material_type" type="text" class="validate" value="<?php echo $material ?>" pattern="[A-Za-z\s]{2,}" name="amaterial">
                    <label for="material_type">Material Type</label>
                </div>

                <div class="col s4" >
                    <button class="waves-effect waves-light btn" value="update" style="margin-top: 5%; margin-left: 5%" name="udMaterial">UPDATE</button>
                </div>
            </form>
        </div>
    </div>
