...
if(isset($_POST["keyword"])){
    ...
    while ($row = $stmt->fetch_object()){
        if($conn->query(" SELECT `catalog`.`catalog_id` FROM `catalog` WHERE `acquisition_number` = '".$row->acquisition_number."' AND `is_missing` = '0' AND `is_borrowed` = '0' AND `date_deleted` IS NULL") > 0) {
            array_push($res, $row);
        }
    }
