...
<div id="container">
    ...
    <div id="content">
        ...
        $stmt = $conn->query("SELECT DISTINCT `acquisition`.`acquisition_number`,`acquisition`.`title`,`acquisition`.`author`,`acquisition`.`edition`,`acquisition`.`quantity` FROM `acquisition` INNER JOIN `keywords`,`subjects`,`catalog` WHERE ((`acquisition`.`title` LIKE '%".$keyword."%' OR `acquisition`.`author` LIKE '%".$keyword."%') OR (`keywords`.`keyword` LIKE '%".$keyword."%' AND `keywords`.`acquisition_number` = `acquisition`.`acquisition_number`) OR (`subjects`.`subject_name` LIKE '%".$keyword."%' AND `acquisition`.`subject_id` = `subjects`.`subject_id`)) AND (catalog.material_type_id = '$material_id') AND (`acquisition`.`date_deleted` IS NULL) ORDER BY `acquisition`.`title`");
        ?>
