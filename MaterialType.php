...
<div id="container">
    ...
    <div id="content">
        ...
        <div class="fixed-action-btn horizontal click-to-toggle">
            <a class="btn-floating btn-large red" href="Add_MaterialType.php">
        ...
        <table class="highlight">
            ...
            $page = ceil($page);
            for($b=1; $b<=$page; $b++){
                ?> <a href="MaterialType.php?page=<?php echo $b;?>"><?php echo $b.' '?></a>  <?php
            }
            $a = 1;
            if(isset($_GET["page"])) {
                $a = $_GET["page"];
            ...
            $stmt2  = $conn->query("SELECT * FROM material_types WHERE `date_deleted` IS NULL LIMIT $page1,10");
            while ($row = $stmt2->fetch_object()){
                echo "<tr><td>".$row->material_type." </td> <td style='display: flex'><form action='Update_MaterialType.php' method='get'><input type='hidden' value='".$row->material_type_id. "' name='material_type_id'>
        ...
        <form action='material_delete.php' method='post' id='$row->material_type_id'><button class='btn-floating material-icons' onclick='showPrompt(`".$row->material_type."`,`".$row->material_type_id."`)' type='button' value='delete' style='border: 0px;color: white;font-size: x-large;'>delete</button><input type='hidden' name='material_type_id' value='".$row->material_type_id."'></form></td></tr>";
            }
            ?>
            </tbody>
        </table>
