...
fputcsv($output, array('Book Title','Author','Edition','Copyright Date', 'Price', 'Date Delivered', 'Date of Shelving', 'Remarks', 'Quantity'));
...
while ($row = $stmt->fetch_object()) {
    fputcsv($output, array($row->title,$row->author,$row->edition,$row->copyright_date, $row->price, $row->date_delivered, $row->date_of_shelving, $row->remarks, $row->quantity));
}
fclose($output);