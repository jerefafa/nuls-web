...
<div id="container">
    ...
    <div id="content">
        ...
        <div class="fixed-action-btn horizontal click-to-toggle">
            <a class="btn-floating btn-large red" href="Add_Subtype.php">
        ...
        <table class="highlight">
            ...
            <?php
            for($b=1; $b<=$page; $b++){
                ?> <a href="Subtype.php?page=<?php echo $b;?>"><?php echo $b.' '?></a>  <?php
            }
            ?>
            </div>
            <?php
            ...
            $stmt2  = $conn->query("SELECT * FROM subtypes WHERE `date_deleted` IS NULL LIMIT $page1, 10");
            while ($row = $stmt2->fetch_object()){
                echo "<tr><td>".$row->subtype." </td> <td style='display: flex'><form action='Update_Subtype.php' method='get'><input type='hidden' value='".$row->subtype_id. "' name='subtype_id'>
        ...
        <form action='subtype_delete.php' method='post' id='$row->subtype_id'><button class='btn-floating material-icons' onclick='showPrompt(`".$row->subtype."`,`".$row->subtype_id."`)' type='button' value='delete' style='border: 0px;color: white;font-size: x-large;'>delete</button><input type='hidden' name='subtype_id' value='".$row->subtype_id."'></form></td></tr>";
            }
            ?>
            </tbody>
        </table>
